...
def scrape_post(playwright):
    ...
    while True:
        paginated_url = f"{CATEGORY_JSON_URL}?page={page_number}"
        ...
        topics = data.get("topic_list", {}).get("topics", [])  # Fixed typo here
        if not topics:
            print("No more pages to scrape")
            break
            
        all_topics.extend(topics)  # Fixed variable name here
        page_number += 1
        print(f"Fetched {len(topics)} topics from page {page_number}.")
    
    filtered_posts = []
    for topic in all_topics:
        created_at = parse_date(topic["created_at"])
        if DATE_FROM <= created_at <= DATE_TO:
            topic_url = f"{BASE_URL}/t/{topic['slug']}/{topic['id']}.json"  # Fixed URL construction
            page.goto(topic_url)
            ...
            reply_counter = {}  # Consistent variable naming
            for post in posts:
